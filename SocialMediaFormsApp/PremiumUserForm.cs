	...
	public partial class PremiumUserForm : Form
	{
		...
		private void PremiumUserForm_Load(object sender, EventArgs e)
		{
			...
			foreach (var username in _userContainer.GetAllUsernames())
			{
				...
				UpdateCommunityAddModsCB.Items.Add(username);
			}
			CreateCommunityAddModsCB.Items.Remove("BotModerator");
			UpdateCommunityAddModsCB.Items.Remove("BotModerator");
		...
		private void UpdateCommunityBT_Click(object sender, EventArgs e)
		{
			...
			UpdateCommunityDescriptionRTB.Clear();
			UpdateCommunityRule1TB.Clear();
			UpdateCommunityRule2TB.Clear();
			UpdateCommunityRule3TB.Clear();
			UpdateCommunityModsLiB.Items.Clear();
		}

		private void UpdateCommuntyAddModBT_Click(object sender, EventArgs e)
		{

			var selectedUser = (string)UpdateCommunityAddModsCB.SelectedItem;
			if (!string.IsNullOrEmpty(selectedUser))
			{
				if (UpdateCommunityModsLiB.Items.Count <= 2 && !UpdateCommunityModsLiB.Items.Contains(selectedUser))
				{
					UpdateCommunityModsLiB.Items.Add(selectedUser);
